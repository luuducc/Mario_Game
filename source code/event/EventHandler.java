...
public class EventHandler {
	...
	public EventHandler(GamePanel gp) {
		...
		event[8] = new EventGate(0, 13, 24, 1, 32, 38);
		event[9] = new EventGate(1, 13, 13, 2, 10, 24);
		...
		if(canTouchEvent == true) {
			for(int i = 0; i < event.length; i++) {
				if(event[i] != null) {
					...
					else if(event[i].getName() == "Gate") teleport((EventGate)event[i], gp.dialogueState, gp.checkMonsterAlive());
					}
					if(hit(event[i], "up")) {
	...
	public void teleport(EventGate e, int gameState, boolean notAccept) {
		if(notAccept == false) {
		...
		if(notAccept == true) {
			...
			gp.ui.setCurrentDialogue("You must kill " + k + " more monsters!");
//			
		}
		canTouchEvent = false;
	}
